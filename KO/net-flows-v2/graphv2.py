...
class Edge:
    ...
    def __str__(self):
        return f'id: {self.id}, src: {self.src.id}, dst: {self.dst.id}, cost: {self.cost}, flow: {self.flow}, ' \
               f'lb: {self.lb}, ub: {self.ub}, address: {hex(id(self))}'
...
class Node:
    ...
    def get_balance(self):
        return sum([e.lb for e in self.in_edges.values()]) - sum([e.lb for e in self.out_edges.values()])

    def __str__(self):
        return f'id: {self.id}, balance: {self.get_balance()}, address: {hex(id(self))}'
...
class Graph:
    ...
    def add_edge(self, src: Node, dst: Node, lb, ub, cost=0, flow=0) -> Edge:
